...
@check_and_update_email_mobile_verification.apply
class Users(db.Document):
    ...
    def get_communication_mobile(self):
        return self.home_mobile if self.communication_address == 'home' else self.business_mobile
...
class Otp(db.Document):
    ...
    meta = {
        'indexes': [
            {'fields':['expiry_date'], 'expireAfterSeconds':0}
        ]
    }
...
class NoticeNewsLetters(db.Document):
    '''
    Used to save News Letters and Notice board events
    '''
...
class CalendarEvents(db.Document):
    ...
    user_type = db.StringField(required=True, max_lenth=100, choices=['admin', 'advisor'])
    created_date = db.DateTimeField(default=datetime.datetime.now)
    modified_date = db.DateTimeField()
