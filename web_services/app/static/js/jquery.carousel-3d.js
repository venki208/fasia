 ...
 */
        ...
        (function() {
          ...
          Carousel3d.prototype._makeOption = function() {
            (function() {
              var $wrapper = $("<div data-children-wrapper></div>")
                .hide()
                .appendTo(this.el);
              var duration = $("<div data-child></div>")
                .hide()
                .appendTo($wrapper)
          ...
          Carousel3d.prototype.appendChild = function(content) {
            this._childrenWrapperObj.appendChild(
              new Child(this._childrenWrapperObj, content)
            );
          };
           ...
           */
          Carousel3d.prototype.appendChildrenWrapper = function(
            childrenWrapperObj
          ) {
          ...
          Carousel3d.prototype.rotate = function(index) {
            ...
            var iFloor = Math.floor(
              this._childrenWrapperObj.currentIndex() - numChildren / 2
            );
            var iCeil = Math.ceil(
              this._childrenWrapperObj.currentIndex() + numChildren / 2
            );
 ...
 */
        (function() {
          ...
          Child.prototype._hideUntilLoad = function() {
            ...
            $(this._contentWrapper).waitForImages(
              function() {
                setTimeout(
                  function() {
                    this._resize();
          ...
          Child.prototype._resize = function() {
            ...
            var horizontalFrameDiff =
              $(this._frame).outerWidth() - $(this._frame).innerWidth();
            var vertialFrameDiff =
              $(this._frame).outerHeight() - $(this._frame).innerHeight();
            var horizontalScale =
              ($(this.el).innerWidth() - horizontalFrameDiff) /
              $(this._content).outerWidth();
            var verticalScale =
              ($(this.el).innerHeight() - vertialFrameDiff) /
              $(this._content).outerHeight();
            var scale = Math.min(horizontalScale, verticalScale);
            var horizontalOffset = Math.floor(
              ($(this.el).innerWidth() -
                horizontalFrameDiff -
                $(this._content).outerWidth() * scale) /
                2
            );
            var verticalOffset = Math.floor(
              ($(this.el).innerHeight() -
                vertialFrameDiff -
                $(this._content).outerHeight() * scale) /
                2
            );
        ...
        (function() {
          ...
          ChildrenWrapper.prototype._resize = function() {
            this._tz =
              $(this.el).outerWidth() /
              2 /
              Math.tan(Math.PI / this._childObjArray.length);
          ...
          ChildrenWrapper.prototype.rotate = function(index) {
            ...
            if (Modernizr.csstransforms3d) {
              for (
                childIndex = 0;
                childIndex < this._childObjArray.length;
                childIndex += 1
              ) {
                childDegree = dDegree * (childIndex - index);
                var transformText = "";
                transformText +=
                  " translateZ(" + -this._tz * (1 + this._spacing) + "px)";
                transformText += " rotateY(" + childDegree + "deg)";
                transformText +=
                  " translateZ(" + this._tz * (1 + this._spacing) + "px)";

                $(this._childObjArray[childIndex].el).css(
                  "transform",
                  transformText
                );
                $(this._childObjArray[childIndex].el).css(
                ...
                $(this._childObjArray[childIndex].el).css(
                  "opacity",
                  Math.cos((Math.PI / 180) * childDegree)
                );
                $(this._childObjArray[childIndex].el).css(
                  "z-index",
                  Math.floor(
                    (Math.cos((Math.PI / 180) * childDegree) + 1) * 100
                  )
                );

                var i =
                  index < 0
                    ? index < this._childObjArray.length * -1
                      ? this._childObjArray.length -
                        ((index * -1) % this._childObjArray.length)
                      : this._childObjArray.length + index
                    : index;
                childIndex ==
                (i > this._childObjArray.length - 1
                  ? i % this._childObjArray.length
                  : i)
                  ? $(this._childObjArray[childIndex].el).attr(
                      "data-child-active",
                      true
                    )
                  : $(this._childObjArray[childIndex].el).removeAttr(
                      "data-child-active"
                    );
              }
            } else {
              ...
              var stepFunc = function(now, tween) {
                if (tween.prop === "_degree") {
                  ...
                  var perspectiveScale =
                    (Math.abs(
                      Math.sin((Math.PI / 180) * (now + halfDegreeRange)) -
                        Math.sin((Math.PI / 180) * (now - halfDegreeRange))
                    ) /
                      (Math.sin((Math.PI / 180) * halfDegreeRange) * 2)) *
                    cos;
                  var heightScale = (cos + 1) / 2;
                  var widthScale = (perspectiveScale + 1) / 2;
                  var dx =
                    ((sin * width) / 2 + ((width * widthScale) / 2) * sin) / 2;
                  ...
                  if (Modernizr.csstransforms) {
                    ...
                    $(tween.elem).css(
                      "top",
                      Math.floor((height - height * heightScale) / 2) + "px"
                    );
                    $(tween.elem).css(
                      "left",
                      (width - width * widthScale) / 2 + dx + "px"
                    );
                    $(tween.elem).css(
                      "filter",
                      "progid:DXImageTransform.Microsoft.Matrix(M11=" +
